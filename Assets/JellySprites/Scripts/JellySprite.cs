...
#region PUBLIC_CLASSES
    ...
    public void Velocity(Vector2 force)
    {
        if (m_ReferencePoints != null)
        {
            if (m_CentralPoint.Body2D)
            {
                m_CentralPoint.Body2D.velocity = force;
            }

            if (m_CentralPoint.Body3D)
            {
                m_CentralPoint.Body3D.velocity = force;
            }
        }
    }
    ...
    public void updateScale(Vector3 scale)
    {
        if (m_ReferencePoints != null)
        {
            foreach (ReferencePoint referencePoint in m_ReferencePoints)
            {
                referencePoint.transform.localScale = scale;
            }
        }
    }
